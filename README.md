...
![Version](https://img.shields.io/badge/dynamic/json?label=Version&color=yellow&style=flat&url=https%3A%2F%2Fraw.githubusercontent.com%2Ftschebbischeff%2Fhabitat-echo%2Frefs%2Fheads%2Fmain%2Fmetadata.json&query=%24.version)
...
[![Audiobookshelf](https://img.shields.io/badge/Audiobookshelf-_?style=flat&logo=audiobookshelf&logoColor=82612C&color=gray&labelColor=gray)](https://audiobookshelf.org/)
 ...
 Provides Audiobookshelf as a platform to manage an audiobook and podcast library.
...
[Original documentation](https://audiobookshelf.org/docs/documentation/server-management/backups)

 - Backup of metadata within the container is scheduled for 01:30 by the default value of `ABS_SETTINGS_BACKUP_SCHEDULE` and written to `/backups`
   - `/backups` must be backed up via volume mounted from `${VOLUME_DIR}/audiobookshelf-backups`
 - Collection of Audiobooks at `/audiobooks` must be backed up via volume mounted from `${VOLUME_DIR}/audiobookshelf-audiobooks`
 - Collection of Podcasts at `/podcasts` must be backed up via volume mounted from `${VOLUME_DIR}/audiobookshelf-podcasts`
